...
public class GenerateParentheses {
    ...
    /**
     * 核心思想在于括号组合合法性判断:
     1. 从左往右对左右括号计数，右括号个数永远不能大于左括号
     2. 左右括号的个数一定要相等
     */

    /**
   * 回溯法解法，模拟求国际象棋用马走遍所有网格的解法
   * 此处走法只有两种，要么左括号，要么右括号。使用核心思想对每个长度到达6的括号进行验证
   * 使用每个堆栈层的index变量作为记住回溯位置的标志物
   * @param n
   * @return
   */
  ...
  public List<String> solve_2(int n) {
    ...
    /**
     * 尾递归解法, 目前最优, 能避免对不符合条件括号组合的验证
     * open < n 对应着左右括号总数相等（即open和close各占一半）
     * close < open 对应右括号数量在任何情况下均不能大于左括号
     * @param n
     * @return
     */
