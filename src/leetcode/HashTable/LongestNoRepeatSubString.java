...
public class LongestNoRepeatSubString {
    ...
    /**
     * 使用哈希表或者数组存储字符到最后一次出现位置的映射
     * 滑动窗口右指针一直向右移动，直到碰到之前遇到过的元素，将窗口的左指针移动到该元素最后一次出现的位置的下一个
     * 注意，左右指针均只向右移动
     * @param str
     * @return
     */
    public static int solve(String str) {
        ...
        for (int i = 0; i < strArray.length; i++) {
            ...
            // 此处，>= front中含有等于的原因是，防止start指针当前所指的元素就是右指针碰到的重复出现元素。此时应该将start指针向后移动一位
            // 此处，front更新要加一的原因是，front指针要跳过那个重复遇见的元素。
